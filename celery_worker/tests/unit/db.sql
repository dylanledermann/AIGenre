...
CREATE TABLE files (
    file_hash CHAR(64) PRIMARY KEY REFERENCES uploads(file_hash) ON DELETE CASCADE,
    file_bytes BYTEA
);
...
CREATE TABLE audio_results (
    ...
    file_hash CHAR(64) NOT NULL UNIQUE REFERENCES uploads(file_hash) ON DELETE CASCADE,
