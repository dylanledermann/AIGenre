...
const WrapperComponent = ({ setError = vi.fn() }) => {
  ...
  return <DragNDrop file={file} setFile={setFile} setError={setError} />;
};
...
const setup = () => {
  ...
  const utils = render(<WrapperComponent setError={setError} />);

  // Gets the dropzone element for the drag and drop file input
  const dropzone = () =>
    screen.getByText('Drop your audio file here').closest('div')!.parentElement!;
...
describe('DragNDrop', () => {
  ...
  describe('drag and drop file', () => {
    it('sets the file when a valid file is dragged and dropped', () => {
      const { dropzone } = setup();
      const file = makeFile('audio.wav');
    ...
    it('handles invalid files correctly', () => {
